...
task("dev-html", () => {
  return src(options.paths.src.base + "/views/**/*.html") // Ensure it captures all views
    .pipe(i18n({
      locales: ['en', 'ar'],
      localeDir: './locales',
      schema: "json"  // Make sure JSON structure is used
    }))
    .pipe(fileinclude({ prefix: "@" }))
    .pipe(dest(options.paths.dist.base)); // Should generate dist/en/ and dist/ar/
});
